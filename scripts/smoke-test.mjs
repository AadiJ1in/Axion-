...
const files = {
  journey: await readFile(new URL("../src/journey-map.js", import.meta.url), "utf8"),
  ...
  poseRuntime: await readFile(new URL("../src/pose-runtime.js", import.meta.url), "utf8"),
  videoFrameScheduler: await readFile(new URL("../src/video-frame-scheduler.js", import.meta.url), "utf8"),
  cameraRuntime: await readFile(new URL("../src/camera-runtime.js", import.meta.url), "utf8"),
  mediapipeConfig: await readFile(new URL("../src/mediapipe-config.js", import.meta.url), "utf8"),
  ...
  therapistMfaMigration: await readFile(new URL("../supabase/migrations/202609030002_therapist_mfa.sql", import.meta.url), "utf8"),
  movementGameAssignmentMigration: await readFile(new URL("../supabase/migrations/202609040001_movement_game_assignments.sql", import.meta.url), "utf8"),
  movementGameGrantMigration: await readFile(new URL("../supabase/migrations/202609040002_movement_game_publish_grant.sql", import.meta.url), "utf8"),
  assignmentRestMigration: await readFile(new URL("../supabase/migrations/202609050001_assignment_rest_intervals.sql", import.meta.url), "utf8"),
  ...
  securityWorkflow: await readFile(new URL("../.github/workflows/security.yml", import.meta.url), "utf8"),
...
const requirements = [
  ...
  [files.journey, "data-world-theme", "touchable session-based patient roadmap"],
  [files.main, "data-roadmap-node", "touchable roadmap session nodes"],
  [files.journey, "journey-region", "phase-specific roadmap world chapters"],
  [files.journey, "journey-node-core", "animated roadmap node atmosphere"],
  ...
  [files.journey, "journey-world", "progress-reactive roadmap regions"],
  [files.main, "roadmapCharacterMarkup", "selected patient character on the current node"],
  [files.journey, "region.nodes.filter(n=>n.state==='complete')", "region completion count"],
  ...
  [files.portal, "publish_patient_plan_v6", "per-exercise rest and mode roadmap publication"],
  [files.portal, "rest_seconds", "rest interval patient/therapist loading"],
  [files.main, "startSetRest", "therapist-scheduled between-set countdown"],
  [files.css, ".prescription-row[hidden]{display:none!important}", "filter result visibility override"],
  [files.movementGameAssignmentMigration, "exercise_assignments_exercise_mode_check", "bounded exercise mode"],
  [files.movementGameAssignmentMigration, "v_key <> 'bodyweight_squat'", "squat-only movement game scope"],
  [files.movementGameGrantMigration, "grant execute on function private.publish_patient_plan_v4", "narrow movement-game publisher grant"],
  [files.assignmentRestMigration, "grant execute on function private.publish_patient_plan_v5", "narrow rest-aware publisher grant"],
  [files.main, "No exercises match these filters", "roadmap assignment empty-filter state"],
  ...
  [files.exerciseCatalog, "External patient-education links are intentionally not shown", "clinic-controlled education boundary"],
  [files.main, "How to do this exercise", "patient exercise instructions"],
  [files.main, "External patient-education links are not shown by Axion", "no third-party patient guide links"],
  ...
  [files.poseRuntime, 'from "@mediapipe/tasks-vision"', "locally bundled pose runtime"],
  [files.pose, "createLocalPoseRuntime", "pose backend abstraction"],
  [files.videoFrameScheduler, "requestVideoFrameCallback", "camera-frame pose scheduling"],
  [files.videoFrameScheduler, "resolveCameraVideoConstraints", "configurable camera capture constraints"],
  [files.cameraRuntime, "OverconstrainedError", "relaxed camera constraint fallback"],
  [files.cameraRuntime, "CameraTimeoutError", "camera startup timeout"],
  [files.pose, "classifyCameraError", "actionable camera error classification"],
  ...
  [files.vite, "bundle-mediapipe-model", "self-hosted verified pose model"],
  [files.vite, "verifiedPoseModelBytes", "build-time pose-model integrity gate"],
  [files.mediapipeConfig, 'appPath: "models/pose-landmarker-lite-float16-v1.task"', "same-origin default pose model path"],
  [files.vite, "bundle-mediapipe-runtime", "self-hosted MediaPipe WebAssembly"],
  [files.vite, "cpSync(mediapipeRuntimeSource", "package-driven MediaPipe runtime bundling"],
  ...
  [files.main, "requireTherapistMfa", "mandatory therapist MFA routing"],
  [files.main, "getAuthenticatorAssuranceLevel", "therapist assurance-level check"],
  [files.main, "Enable therapist MFA", "therapist authenticator enrollment"],
  ...
  [files.rlsIntegrationTest, "Password-only therapist session retained clinical authorization", "therapist MFA regression test"],
  ...
  [files.therapistMfaMigration, "p.role <> 'therapist'", "therapist-only MFA policy"],
  [files.therapistMfaMigration, "'aal' = 'aal2'", "AAL2 database enforcement"],
  [files.securityWorkflow, "security-extended", "scheduled CodeQL security analysis"],
  [files.securityWorkflow, "npm audit --omit=dev", "continuous dependency vulnerability gate"],
  [files.dependabot, "interval: weekly", "weekly dependency update review"],
