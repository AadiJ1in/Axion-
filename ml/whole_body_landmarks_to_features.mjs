...
if (outputIndex < 0 || !args[outputIndex + 1]) {
  console.error("Usage: node ml/whole_body_landmarks_to_features.mjs --output <features.csv>");
  process.exit(2);
}
...
const QUALITY_COLUMNS = [
  ...
  "frame_coverage",
  "feature_coverage",
  ...WHOLE_BODY_REGIONS.map((region) => `coverage_${region}`),
];
const COLUMNS = [...META_COLUMNS, ...QUALITY_COLUMNS, ...WHOLE_BODY_FEATURES_V1];
...
function ensureHeader() {
  ...
  if (firstLine !== EXPECTED_HEADER) {
    throw new Error("existing WBF CSV schema does not match WHOLE_BODY_FEATURES_V1; use a new output path");
  }
}
...
function addFrame(event) {
  ...
  const frame = extractWholeBodyFrame({
...
function finishVideo(event) {
  ...
  const features = {};
  let present = 0;
  for (const name of WHOLE_BODY_FEATURES_V1) {
  ...
  appendRow({
    ...
    feature_coverage: WHOLE_BODY_FEATURES_V1.length
      ? Math.round((present / WHOLE_BODY_FEATURES_V1.length) * 1000) / 1000
      : null,
    ...Object.fromEntries(WHOLE_BODY_REGIONS.map((region) => [`coverage_${region}`, summary.regionCoverage?.[region] ?? null])),
    ...features,
  });
  current = null;
}
...
try {
  ...
  console.error(`whole_body_landmarks_to_features failed at input line ${lineNumber}:`, error instanceof Error ? error.message : error);
  process.exit(1);
}
