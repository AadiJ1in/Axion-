  ...
  if (connection?.status === "active") {
    ...
    if (plan) {
      const [assignmentResult, roadmapResult, nodeResult, nodeAssignmentResult, completionResult] = await Promise.all([
        client.from("exercise_assignments").select("id, plan_id, exercise_key, display_name, sequence, tracking_mode, exercise_mode, rest_seconds, target_sets, target_repetitions, duration_seconds, instructions, status").eq("plan_id", plan.id).eq("status", "active").order("sequence"),
  ...
  const assignments = planIds.length
    ? await throwIfError(
      await client.from("exercise_assignments")
        .select("id, plan_id, exercise_key, display_name, sequence, tracking_mode, exercise_mode, rest_seconds, target_sets, target_repetitions, duration_seconds, instructions, status")
        .in("plan_id", planIds)
        .order("sequence"),
...
export async function createPersonalPlan(client, therapistId, patientId, input) {
  ...
  return throwIfError(
    await client.rpc("publish_patient_plan_v5", {
      p_patient_id: patientId,
