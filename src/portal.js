import { getAdventureDefinition } from "./adventure-definitions.js";
...
export async function loadPatientWorkspace(client, userId) {
  const [profileResult, relationshipResult, sessionsResult, safetyEventsResult] = await Promise.all([
    client.from("profiles").select("id, display_name, role, onboarding_version, onboarding_completed_at, recovery_xp, level, streak_days, avatar_key").eq("id", userId).single(),
    client.from("therapist_patients").select("therapist_id, patient_id, status, patient_confirmed_at, therapist_verified_at, invitation_id").eq("patient_id", userId).order("created_at", { ascending: false }),
    client.from("exercise_sessions").select("id, assignment_id, roadmap_node_id, exercise_key, repetitions, duration_seconds, movement_summary, completed_at, created_at").eq("patient_id", userId).order("created_at", { ascending: false }).limit(50),
    client.from("patient_safety_events").select("id, patient_id, assignment_id, session_id, client_session_id, exercise_key, set_number, rep_number, event_type, pain_score, comment, paused_session, occurred_at, created_at").eq("patient_id", userId).order("occurred_at", { ascending: false }).limit(20),
  ]);

  const profile = await throwIfError(profileResult, "Could not load your profile");
  const relationships = await throwIfError(relationshipResult, "Could not load your care-team connection") || [];
  ...
  const [therapistResult, planResult] = await Promise.all([
    connection?.therapist_id
  ...
  const therapist = therapistResult.error ? null : therapistResult.data;
  ...
  if (plan) {
    const [assignmentResult, roadmapResult, nodeResult, completionResult] = await Promise.all([
    ...
    const nodeIds = roadmapNodes.map((node) => node.id).filter(Boolean);
    if (nodeIds.length) {
      const nodeAssignmentResult = await client.from("roadmap_node_assignments")
        .select("roadmap_node_id, assignment_id, sequence")
        .in("roadmap_node_id", nodeIds)
        .order("sequence");
      roadmapNodeAssignments = await throwIfError(nodeAssignmentResult, "Could not load session exercises") || [];
    }
  }
