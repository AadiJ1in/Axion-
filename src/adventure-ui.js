export function adventureMarkup(mapping, targetReps, assignment, escapeHtml) {
 ...
 return `<section class="movement-game-card adventure-card" data-scene="${mapping.scene}">
  <div class="movement-game-heading"><div><span class="game-kicker">AXION · THE FRACTURED WORLD</span><h3>${e(mapping.title)}</h3><p>${e(mapping.instruction)}</p></div><div class="mode-switch"><button data-movement-mode="standard">Standard view</button><button class="active" data-movement-mode="game">Adventure</button></div></div>
  <div id="movement-game-stage" class="movement-game-stage active">
   ...
   <div class="adventure-viewport"><canvas id="adventure-canvas" aria-label="${e(mapping.title)} movement-controlled world"></canvas>
