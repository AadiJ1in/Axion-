...
function start(key='bodyweight_squat') {
  ...
  controller=createMovementGameController({exerciseKey:key,targetReps:4,onState:s=>{
  ...
  root.querySelector('#motion').oninput=e=>controller.consume({type:MOVEMENT_EVENT.MOVEMENT_PROGRESS,progress:Number(e.target.value)/100,stage:Number(e.target.value)>10?'down':'up',side:'left'});
