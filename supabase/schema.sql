...
-- Public signup creates a patient profile. Client-supplied role metadata is ignored.
...
begin
  ...
  values (
    new.id,
    coalesce(nullif(trim(new.raw_user_meta_data ->> 'display_name'), ''), 'Demo Patient'),
    'patient'
  );
  return new;
end;
...
-- Role lookup used by RLS. The function returns only the caller's server-controlled profile role.
...
-- Profiles: a patient can read only their own profile.
create policy "profiles_read_self"
on public.profiles for select
to authenticated
using (id = auth.uid());

-- Therapists can read profiles for this proof of concept.
create policy "profiles_read_therapist"
on public.profiles for select
to authenticated
using (public.current_app_role() = 'therapist');

-- No client update policy is intentionally provided for role fields.
-- Administrative role changes must use the dashboard SQL editor/service role.
...
create policy "sessions_read_therapist"
on public.exercise_sessions for select
to authenticated
using (public.current_app_role() = 'therapist');
...
-- After creating a therapist through Supabase Auth, promote that account manually:
